...
public class SimModelImp extends BaseModel{

    private Context context;
    private ServiceApi serviceApi;
    private CompositeSubscription mCompositeSubscription;

    public SimModelImp(Context mContext) {
        super();
        context = mContext;
    ...
    public void loadCode(String token , final IBaseRequestCallBack<Sim> iBaseRequestCallBack) {
        mCompositeSubscription.add(serviceApi.getSim(token)
                .observeOn(AndroidSchedulers.mainThread())//指定事件消费线程
                .subscribeOn(Schedulers.io())   //指定 subscribe() 发生在 IO 线程
                .subscribe(new Subscriber<Sim>() {
